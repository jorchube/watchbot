...
class TestMotionWatcher:
    def test_it_opens_video_stream_on_creation(
        self, mock_stream_motion_detector, mock_video_writer
    ):
        MotionWatcher(
            "rtsp://video:stream/uri", mock_stream_motion_detector, mock_video_writer
        )

        mock_stream_motion_detector.open_stream.assert_called_once_with(
            "rtsp://video:stream/uri"
        )
    ...
    def test_it_does_nothing_when_motion_is_not_detected(
        ...
        mock_stream_motion_detector.get_frame_with_motion_detected.return_value = (
            stream_frame,
            is_motion_detected,
        )
    ...
    def test_it_does_not_starts_a_recording_when_the_last_one_was_less_than_3_minutes_ago(
        self, mock_stream_motion_detector, mock_video_writer
    ):
        motion_recorder = MotionWatcher(
            "rtsp://video:stream/uri", mock_stream_motion_detector, mock_video_writer
